...
const socket = io(RootAPI, { transports: ["websocket"] });
...
function Chat(props) {
  ...
  const fetchData = async () => {
    ...
    setAllRoom(result.allRoom);
  };
  useEffect(() => {
    fetchData();
  }, []);

  // const getRoomsByKey = async (key) => {
  //   const result = await ChatRoomsAPI.getRoomByKey(key);
  //   setAllRoom(result.rooms);
  // };

  useEffect(() => {
    if (!keySearch) setRooms([...allRoom]);
    else {
      let results = allRoom.filter(
        (room) =>
          room?._id.includes(keySearch) || room?.fullname?.includes(keySearch)
      );
      setRooms([...results]);
    }
  }, [keySearch, allRoom]);
  ...
  const handlerSend = () => {
    ...
    const postData = async () => {
      try {
        ...
        setTextMessage("");
        //Khi text === /delete sẽ xoá phòng chat này
        if (res.status === 203) {
          // console.log(res);
          alert(res.message);
          setMessage([]);
            ...
            <div className="card">
              <div className="row no-gutters">
                <div className="col-lg-3 col-xl-2 border-right">
                  ...
                  <div
                    className="mailbox scrollable position-relative overflow-auto"
                ...
                <div className="col-lg-9 col-xl-10">
                  ...
                  >
                    <ul className="chat-list list-style-none px-3 pt-3">
                      ...
                      <li ref={messagesEndRef} key="zz9"></li>
                    </ul>
                  </div>
